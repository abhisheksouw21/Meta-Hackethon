...
from models import (
    Action, Observation, ExecuteSQL, ReadFile, WriteFile, SubmitTask, 
...
class ComplianceEnv:
    ...
    def _grade_task(self) -> float:
        ...
        if self.current_task == "easy":
            ...
            if cursor.fetchone()[0] == 0: score += 0.34

        elif self.current_task == "hard":
            # HARD: Data Retention Audit (Find logs older than 5 years)
            # The AI should create a new file called /violation_report.txt
            report = self.filesystem.get("/violation_report.txt", "")
            
            # tx_2018.log is a violation and should be flagged
            if "tx_2018.log" in report:
                score += 1.0
                
            # tx_2024.log is safe and should NOT be flagged
            if "tx_2024.log" in report:
                score -= 0.5 # Penalty for false positive
            
        # Ensure the final score strictly stays within the 0.0 to 1.0 bounds required by OpenEnv
